...
public class Projectservice {
	...
	public String push(Project p) {
		String sql="select count(*) from mss.resources where eid=? and employeename=?";
		@SuppressWarnings("deprecation")
		int obj= jt.queryForObject(sql,
			    new Object[] 
			    {p.getEid(),p.getEmployeename()},Integer.class);
		
	if(obj>0)
	{
		String sql1="select count(*) from mss.project where pid=?";
		@SuppressWarnings("deprecation")
		int obj2=jt.queryForObject(sql1, new Object[]{
			 p.getPid()}, Integer.class);
		
		if(obj2>0)
		{
			System.out.println("insertup");
			String sql2="insert into mss.projectdetails values(?,?,?,?,?,?,?)";
			String sql5="select status from mss.project where pid= ?";
			System.out.println(sql5);
			@SuppressWarnings("deprecation")
			String s=jt.queryForObject(sql5, new Object[]
					{p.getPid()}, String.class);
			System.out.println(s);
			int i= jt.update(sql2,p.getPid(),p.getEid(),p.getEmployeename(),p.getCreatedby(),p.getStartdate(),p.getEnddate(),s);
              if(i>0)
              {
